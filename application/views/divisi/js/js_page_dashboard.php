<script>
  	$(document).ready(function() {
		$("#dtDiv").DataTable({
			"processing": true,
			"ajax": {
				"url": "<?= base_url('divisi/dashboard/list_data/') ?>",
				"type": "POST"
			}
		});
		...
		$('#FrmDiv').submit(function(e) {
			...
			swal({
				...
				if(Oke) {
					$.ajax({
						type: "POST",
						url: "<?= base_url('divisi/dashboard/simpan/') ?>",
		...
		$(document).on('click','#edit',function() {
			...
			jQuery.ajax({
				type: "POST",
				url: "<?= base_url('divisi/dashboard/get_data/') ?>",
				dataType: 'json',
				data: { divisi_id: $(this).attr("data") },
		...
		$(document).on('click','#hapus',function() {
			swal({
				...
				if(Oke) {
					$.ajax({
						type: "POST",
						url: "<?= base_url('divisi/dashboard/hapus/') ?>",
						data: { divisi_id: $(this).attr("data") },
						timeout: 5000,
						success: function(response) {
							var data = JSON.parse(response);
